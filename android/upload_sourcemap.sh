...
if [ ${INSTABUG_APP_TOKEN} == "YOUR_APP_TOKEN" ]; then
    cmd /C echo "Instabug: Looking for Token..."
...
if [ ! "${INSTABUG_APP_TOKEN}" ] || [ -z "${INSTABUG_APP_TOKEN}" ] || [ "${INSTABUG_APP_TOKEN}" == "YOUR_APP_TOKEN" ];then
    cmd /C echo "Instabug: err: INSTABUG_APP_TOKEN not found. Make sure you've added the SDK initialization line Instabug.startWithToken Or added it to the environment variable in the gradle"
    exit 0
else
    cmd /C echo "Instabug: Uploading files..."
